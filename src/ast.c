...
static void free_stmt(Stmt *s){ while(s){ Stmt *n=s->next; if(s->type==STMT_ASSIGN){ free(s->data.assign.var); free_expr(s->data.assign.expr);} else if(s->type==STMT_IF){ free_expr(s->data.if_stmt.cond); free_stmt(s->data.if_stmt.then_branch); free_stmt(s->data.if_stmt.else_branch);} else if(s->type==STMT_WHILE){ free_expr(s->data.while_stmt.cond); free_stmt(s->data.while_stmt.body);} else if(s->type==STMT_BLOCK){ free_stmt(s->data.block.stmts);} free(s); s=n; } }
